...
export default function Cotizaciones() {
  ...
  const abrirSelector = () => { setShowSelector(true); setQ(''); setContactos([]); };
      ...
      {showSelector && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50" onClick={() => setShowSelector(false)}>
          <div onClick={e => e.stopPropagation()} className="bg-white rounded-lg p-6 w-full max-w-lg max-h-[80vh] flex flex-col">
            <h2 className="font-semibold text-ht-navy text-lg mb-1">Nueva cotización</h2>
            <p className="text-xs text-gray-500 mb-3">Elige el contacto o empresa; el negocio se crea automáticamente con los datos de la cotización.</p>
