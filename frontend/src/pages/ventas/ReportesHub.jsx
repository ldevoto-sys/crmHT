...
export default function ReportesHub({ vistaInicial = 'pipeline' }) {
    ...
    <div>
      <div className="inline-flex border border-gray-300 rounded overflow-hidden mb-5">
        <button onClick={() => setVista('pipeline')}
          className={`text-sm font-medium px-4 py-2 ${vista === 'pipeline' ? 'bg-ht-accent text-white' : 'bg-white text-gray-600'}`}>
          Pipeline
        </button>
        <button onClick={() => setVista('softland')}
          className={`text-sm font-medium px-4 py-2 border-l border-gray-300 ${vista === 'softland' ? 'bg-ht-accent text-white' : 'bg-white text-gray-600'}`}>
          Comercial (Softland)
        </button>
      </div>
