...
export default function ConfigPostventaEtapas() {
  const [etapas, setEtapas] = useState([]);
  const [error, setError] = useState(''); const [msg, setMsg] = useState('');
  const [nuevo, setNuevo] = useState('');

  const cargar = async () => {
    try { setEtapas((await api.get('/postventa/etapas')).data); }
  ...
  const guardar = async (e) => {
    setError(''); setMsg('');
    try {
      await api.put(`/postventa/etapas/${e.id}`, { nombre: e.nombre, activo: e.activo });
  ...
  const eliminar = async (e) => {
    ...
    setError(''); setMsg('');
    try { await api.delete(`/postventa/etapas/${e.id}`); cargar(); }
  ...
  const crear = async (ev) => {
    ...
    try {
      await api.post('/postventa/etapas', { nombre: nuevo });
      setNuevo(''); cargar();
  ...
  const moverOrden = async (etapa, direccion) => {
    ...
    try {
      await Promise.all([
        api.put(`/postventa/etapas/${etapa.id}`, { nombre: etapa.nombre, activo: etapa.activo, orden: vecino.orden }),
        api.put(`/postventa/etapas/${vecino.id}`, { nombre: vecino.nombre, activo: vecino.activo, orden: etapa.orden }),
      ]);
      cargar();
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-ht-navy mb-1">Configuración de Postventa</h1>
      <p className="text-gray-500 text-sm mb-6">Etapas del tablero de Postventa. "Resuelto" y "Rechazado" no se pueden eliminar.</p>
