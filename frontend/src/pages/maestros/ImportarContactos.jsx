...
export default function ImportarContactos() {
  ...
  const analizar = async e => {
    ...
    try {
      const { data } = await api.post('/contactos/importar/preview', fd);
      setPreview(data);
  ...
  const confirmar = async () => {
    ...
    const fd = new FormData(); fd.append('archivo', archivo);
    try {
      const { data } = await api.post('/contactos/importar/confirmar', fd);
  ...
  const descargarPlantilla = async () => {
    const { data } = await api.get('/contactos/importar/plantilla', { responseType: 'blob' });
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-bold text-ht-navy mt-2 mb-2">Importar contactos</h1>
      <p className="text-gray-500 text-sm mb-4">
        Sube un CSV con las columnas de la plantilla. Se matchea por <strong>teléfono</strong> (o email si no hay teléfono):
        crea los nuevos y completa datos de los existentes. Los teléfonos se normalizan a formato +56.
      </p>
      ...
      {resultado && (
        <div className="p-4 bg-green-50 border border-green-200 rounded text-green-800 text-sm">
          ...
          <ul className="list-disc pl-5 mt-1">
            <li>Contactos insertados: {resultado.insertados}</li>
            <li>Contactos actualizados: {resultado.actualizados}</li>
            <li>Empresas creadas: {resultado.empresas_referenciadas}</li>
          </ul>
          <Link to="/contactos" className="inline-block mt-3 text-ht-navy underline">Ver contactos</Link>
        </div>
      )}
          ...
          <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead className="text-gray-500"><tr>
                ...
                <th className="text-left px-4 py-1 font-medium">Email</th>
              ...
              <tbody>
                {preview.muestra.map((m, i) => (
                  <tr key={i} className="border-t border-gray-100">
                    ...
                    <td className="px-4 py-1 text-gray-600">{m.empresa || '—'}</td>
