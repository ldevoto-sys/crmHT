...
// fecha_compromiso llega del backend como timestamp completo (el driver de
// Postgres convierte DATE a un objeto Date, que se serializa como ISO
// completo), no como "AAAA-MM-DD" — hay que recortar antes de parsear.
...
// A diferencia de Postventa, aquí no hay concepto de "vendedor dueño del
// caso" (no nace de un pipeline de ventas): todos los roles con acceso al
// módulo ven y gestionan cualquier caso por igual.
export default function ServicioTecnico() {
  ...
  const cargar = async () => {
    try { setCasos((await api.get('/servicio-tecnico')).data); }
    catch { setError('No se pudieron cargar los casos de servicio técnico.'); }
  };
  useEffect(() => { cargar(); }, []);
  useEffect(() => {
  ...
  const mover = async (caso, etapa) => {
    if (caso.etapa_id === etapa.id) return;
    try { await api.put(`/servicio-tecnico/${caso.id}/etapa`, { etapa_id: etapa.id }); cargar(); }
  ...
  const casosFiltrados = filtroSla === 'todos' ? casos : casos.filter(c => slaEstado(c.fecha_compromiso) === filtroSla);
  const porEtapa = id => casosFiltrados.filter(c => c.etapa_id === id);
  // Casos creados antes de que existiera alguna etapa "abierta" quedan sin
  // etapa_id — se muestran aparte para que nunca queden invisibles.
  ...
  const guardarGestion = async (caso, campos) => {
    try { await api.put(`/servicio-tecnico/${caso.id}`, campos); cargar(); setDetalle(null); }
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        <h1 className="text-2xl font-bold text-ht-navy">Servicio Técnico</h1>
      ...
      <div className="flex gap-3 overflow-x-auto pb-4">
        {sinEtapa.length > 0 && (
          <div className="flex-shrink-0 w-72 bg-amber-50 border border-amber-200 rounded-lg p-2">
            ...
            <p className="text-[11px] text-amber-700 px-1 mb-2">Define las etapas en Config Servicio Técnico y muévelos desde aquí.</p>
            <div className="space-y-2 min-h-[40px]">
              {sinEtapa.map(c => (
                <TarjetaCaso key={c.id} c={c} etapas={etapas} etapaActualId={null}
                  onDragStart={setDrag} onClick={setDetalle} onMover={mover} />
              ))}
            </div>
          </div>
        )}
      ...
      {detalle && (
        <DetalleCaso caso={detalle} tecnicos={tecnicos}
...
function TarjetaCaso({ c, etapas, etapaActualId, onDragStart, onClick, onMover }) {
  const estadoSla = slaEstado(c.fecha_compromiso);
  const estilo = ESTILO_SLA[estadoSla] || ESTILO_SLA.normal;
  return (
    <div draggable onDragStart={() => onDragStart(c)}
      onClick={() => onClick(c)}
      className={`bg-white rounded-md border border-gray-200 ${estilo.borde} p-3 hover:border-ht-accent cursor-pointer cursor-move`}>
      ...
      <div className="flex items-center justify-between mt-2">
        ...
        {c.fecha_compromiso && (
          <span className={`text-[11px] ${estilo.texto}`}>{estilo.label ? `${estilo.label} · ` : 'Compromiso '}{fecha(c.fecha_compromiso)}</span>
        )}
      </div>
...
function NuevoCaso({ negocioIdInicial, onClose, onCreado }) {
  ...
  const [form, setForm] = useState({ titulo: '', descripcion: '', prioridad: 'media', detalle_equipo: '', fecha_compromiso: '' });
  ...
  const crear = async e => {
    ...
    } else if (!negocio) { setError('Selecciona el negocio de origen (la venta a la que corresponde este caso), o marca "¿Sin venta asociada?".'); return; }
    setCreando(true);
    try {
      const { data: caso } = await api.post('/servicio-tecnico', {
      ...
      for (const foto of fotos) {
        ...
        await api.post(`/servicio-tecnico/${caso.id}/adjuntos`, fd, { headers: { 'Content-Type': 'multipart/form-data' } });
      }
      onCreado();
  ...
  return (
    <Modal onClose={onClose}>
      <h2 className="font-semibold text-ht-navy text-lg mb-3">Nuevo caso de servicio técnico</h2>
      ...
      <form onSubmit={crear} className="space-y-3">
        ...
        <div>
          ...
          <input required value={form.titulo} onChange={e => setForm({ ...form, titulo: e.target.value })} placeholder="Ej: Bomba no enciende, revisión técnica"
            className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ht-accent" />
        </div>
        <div>
        ...
        <div>
          <label className="block text-sm text-gray-700 mb-1">Equipo/bomba (opcional)</label>
          {productoSel ? (
            <div className="flex items-center justify-between border border-gray-300 rounded px-3 py-2 text-sm">
...
function DetalleCaso({ caso, tecnicos, onClose, onGuardar }) {
  const [prioridad, setPrioridad] = useState(caso.prioridad);
  const [tecnicoId, setTecnicoId] = useState(caso.tecnico_asignado_id || '');
  const [fechaCompromiso, setFechaCompromiso] = useState(caso.fecha_compromiso ? caso.fecha_compromiso.slice(0, 10) : '');
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <div className="space-y-3 border-t border-gray-100 pt-3">
        ...
        <div>
          <label className="block text-sm text-gray-700 mb-1">Fecha de compromiso</label>
          <input type="date" value={fechaCompromiso} onChange={e => setFechaCompromiso(e.target.value)}
            className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ht-accent" />
        </div>
        <button onClick={() => onGuardar(caso, { prioridad, tecnico_asignado_id: tecnicoId || null, fecha_compromiso: fechaCompromiso || null })}
...
function AdjuntosCaso({ casoId }) {
  ...
  const ver = async adj => {
    try {
      const { data } = await api.get(`/servicio-tecnico/adjuntos/${adj.id}/archivo`, { responseType: 'blob' });
      window.open(URL.createObjectURL(data), '_blank');
    } catch { setError('No se pudo abrir el archivo.'); }
  };

  const eliminar = async adjId => {
    try { await api.delete(`/servicio-tecnico/adjuntos/${adjId}`); cargar(); }
        ...
        <ul className="space-y-1.5 mb-2">
          {adjuntos.map(a => (
            <li key={a.id} className="flex items-center justify-between text-sm border border-gray-100 rounded px-2 py-1.5">
              ...
              {(user?.id === a.subido_por_id || user?.rol === 'administrador' || user?.rol === 'jefe_comercial') && (
