...
router.put('/:id/etapa', async (req, res) => {
  try {
    const { etapa_id, causa_no_cierre_id, causa_no_cierre_detalle, tipo_trabajo } = req.body;
    ...
    // que se fija) o ya estar en el negocio de una entrada anterior.
    const entraAAceptado = etapa.nombre.toLowerCase() === 'aceptado';
    const tipoTrabajoFinal = tipo_trabajo || negocio.tipo_trabajo;
    if (entraAAceptado) {
      if (!tipoTrabajoFinal) return res.status(400).json({ error: 'El tipo de trabajo es obligatorio para pasar a "Aceptado"' });
      if (!TIPOS_TRABAJO.includes(tipoTrabajoFinal)) return res.status(400).json({ error: 'Tipo de trabajo inválido' });
    }
    ...
    if (entraAAceptado) {
      await ot.crearOTSiNoExiste({ id: negocio.id, tipo_trabajo: tipoTrabajoFinal }, db, req.user.id);
    }
