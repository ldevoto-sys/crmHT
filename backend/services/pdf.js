...
// Igual que generarCotizacionPDFBuffer, pero para la OT — hace falta como
// Buffer (no solo a stream) para poder fusionarla dentro del informe de
// Postventa (ver generarInformePostventaPDF y routes/postventa.js).
async function generarOTPDFBuffer(data) {
  const stream = new PassThrough();
  const chunks = [];
  stream.on('data', chunk => chunks.push(chunk));
  const listo = new Promise((resolve, reject) => {
  ...
  await generarOTPDF(data, stream);
  return listo;
}
...
// La cotización, la OT y los adjuntos que sean PDF NO se dibujan acá — se
// fusionan aparte como páginas completas con pdf-lib (ver routes/postventa.js,
// PDFKit no puede importar páginas de un PDF ya existente). Esta función solo
...
// junto a la cotización/OT/adjuntos PDF, no puede pipear directo a la
// respuesta como sí hacen generarCotizacionPDF/generarOTPDF).
...
module.exports = {
  generarCotizacionPDF, generarCotizacionPDFBuffer, generarOTPDF, generarOTPDFBuffer,
