...
async function avanzarRecontactosPendientes() {
  ...
  const cfg = await db.get('SELECT recontacto_respeta_horario, bot_activo FROM whatsapp_bot_config WHERE id = 1');
  if (!cfg?.bot_activo) return { procesados: 0, enviados: 0, cerrados: 0 };
